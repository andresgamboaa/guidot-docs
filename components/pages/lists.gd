	...
	Gui.smoothScrollbox({preset="scroll-cc-exp",id="scroll"},[
		Gui.margin({preset="margin-cc-exp-h", minimum_size=Vector2(0,0)},[
			Gui.vbox({preset="vbox-cc-exp-h"},[
				...
				Gui.margin({minimum_size=Vector2(0,20)}),
				Gui.label({preset="paragraph", text="The hash of the array has to be passed to ensure the BasicComponent(vbox) will be updated correctly each time the array changes."}),
				Gui.margin({minimum_size=Vector2(0,20)}),
				Gui.label({preset="paragraph", text="It is very important to add a key with an unique value to each item."}),
				Gui.margin({minimum_size=Vector2(0,20)}),
				Gui.rich_label({preset="rich-paragraph", text='Next: [url=Conditional rendering]Conditional rendering[/url]', on_meta_clicked=handle_meta_clicked}),
			]),
		])
	])
