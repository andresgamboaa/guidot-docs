	...
	Gui.scrollbox({preset="scroll-cc-exp",id="scroll"},[
		Gui.margin({preset="margin-cc-exp-h", minimum_size=Vector2(0,0)},[
			Gui.vbox({preset="vbox-cc-exp-h"},[
				...
				Gui.label({preset="paragraph", text="To create the UI with Guidot, make a scene with a control node and add the RootComponent to it."}),
				Gui.margin({minimum_size=Vector2(0,20)}),
				...
				Gui.margin({minimum_size=Vector2(0,20)}),
				Gui.scrollbox({preset="expand-h",minimum_size=Vector2(0,80)},[
				...
				Gui.margin({minimum_size=Vector2(0,20)}),
				Gui.rich_label({preset="rich-paragraph", text='Next: [url=]State[/url]'}),
			]),
		])
	])
